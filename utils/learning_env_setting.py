...
def argparser(epochs, learning_rate, train_batch_size):
    ...
    parser.add_argument('-a', type=str, default=None,
                        help='a string for activation function')
    ...
    return exp_idx, epochs, learning_rate, training_batch_size, activation


def dir_setting(exp_name, CONTINUE_LEARNING = False):

    cp_path = os.path.join(os.getcwd(), exp_name)
...
def continue_setting(CONTINUE_LEARNING, path_dict, model=None):
    ...
    if CONTINUE_LEARNING == True:
        ...
        losses_acc_np = np.load(losses_accs_path + '/losses_accs.npz')
        losses_accs = dict()
        for k, v in losses_acc_np.items():
